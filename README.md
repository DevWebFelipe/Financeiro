...
Próxima fase: **Fase 13 — Parcelamento / Negociação / Renegociação de fatura**. Status: `CONTRATO APROVADO — IMPLEMENTAÇÃO PENDENTE` (`docs/24` §19.4). A Fase 9 está **CONCLUÍDA** e **APROVADA**. Não implementar Refresh Token, `payments.type`, Agreement/parcelamento de fatura, relatórios/PDF, frontend financeiro, auditoria genérica nem `POST /invoices/{id}/close` sem autorização explícita da etapa de implementação.

Não implementar Refresh Token, logout, OAuth, MFA, roles, rate limiting, frontend financeiro, Agreement/parcelamento de fatura, relatórios/PDF, auditoria genérica nem `payments.type` sem autorização. A edição de parcela já em fatura (§269.2.7) permanece **deferida**. Fechados: §269.3, §269.4, **§269.5** (contrato Fase 13).
