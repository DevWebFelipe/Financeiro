...
Contrato oficial documentado (`docs/24` §19.8 / `docs/25` §67). **Não implementado.** Não existe endpoint no código nesta etapa.

Visão derivada de `Income`. Sem tabela `receivables`. Rota prevista: `GET /api/v1/receivables`. Data: `expectedDate` (obrigatória; sem alias `dueDate`). Padrão: somente `EXPECTED`. `OVERDUE` derivado. Resumo no mesmo GET, respeitando filtros.
...
| Contas a receber (Fase 17 Parte 1) | Contrato documentado — **não implementado**; visão derivada `GET /api/v1/receivables`; `expectedDate`; sem tabela; Parte 2 (movimentações) fora |
...
Fase 17 — Contas a receber — contrato da Parte 1 documentado — não implementada
```
...
Fase 13 — **CONCLUÍDA E APROVADA** (`docs/24` §19.4 / RN254). **Fase 14** — **`CONCLUÍDA E APROVADA`**. **Fase 15** — Metas: contrato em `docs/24` §19.6 / `docs/25` §54E — implementação concluída, **aguardando auditoria final**. **Fase 16** — Contas a pagar: contrato em `docs/24` §19.7 / `docs/25` §66 — **`CONCLUÍDA E APROVADA`**; **não** criar tabela `payables`. **Fase 17 Parte 1** — Contas a receber: contrato em `docs/24` §19.8 / `docs/25` §67 — **documentado, não implementado**; **não** criar tabela `receivables`. Não implementar Refresh Token, `payments.type`, relatórios/PDF, frontend financeiro, auditoria genérica, extrato `/statement` nem `POST /invoices/{id}/close` sem autorização.
