...
- Pacote de consulta da Fase 20 (`docs/24` §19.12 — **CONCLUÍDA E APROVADA**; auditoria final **APROVADA COM RESSALVAS**): `reports` — visões derivadas `GET /api/v1/reports/*` + PDF (OpenPDF 3.0.5); **sem** tabela/entidade JPA; **sem** migration. D-F20-01 a D-F20-16 **fechadas** e **implementadas**. **Não** criar tabela `reports`. Paths antigos `expenses-by-category`, `income-by-category`, `expenses-by-responsible`, `expenses-by-card` estão **SUPERADOS**.
...
- Relatórios na V1 (Fase 20 — contrato `docs/24` §19.12 / `docs/25` §76 — **CONCLUÍDA E APROVADA**; auditoria final **APROVADA COM RESSALVAS**): visões derivadas `GET /api/v1/reports/*` + PDF (OpenPDF 3.0.5). Sem tabela `reports`. Sem migration. Sem reabrir Dashboard nem `GET /projections`. Filtro de responsável no relatório de fatura (cartão de terceiro). **Não** usar `payments.type`.
- PDF: **OpenPDF** 3.0.5 (`com.github.librepdf:openpdf`). Implementado na Fase 20.
- Gráficos: **Apache ECharts** (frontend; fora da Fase 20).
